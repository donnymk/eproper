...
if($_SESSION['nama'] != $baris['asal_peserta']){
    ...
    $query="SELECT nip, namadiklat, judul, DATE_FORMAT(tglsubmit,'%d/%m/%Y %T') AS tglsubmit, (SELECT nama FROM peserta WHERE nip=(SELECT nip FROM inovasi WHERE id='".$id."')) AS nama, (SELECT jabatan FROM peserta WHERE nip=(SELECT nip FROM inovasi WHERE id='".$id."')) AS jabatan, (SELECT skpd FROM peserta WHERE nip=(SELECT nip FROM inovasi WHERE id='".$id."')) AS skpd FROM inovasi WHERE id='".$id."'";
